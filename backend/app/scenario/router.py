...
async def create_scenario_endpoint(
    ...
    :param user: Authenticated user creating the scenario
    :type user: EnUserDB
    ...
    scenario = create_scenario(scenario_data=scenario_data, user=user)
...
async def read_scenarios_endpoint(
    ...
    if not user.check_project_rights(project_id=project_id):  # db=db):
        ...
        response_data = [
            scenario.model_dump(exclude={"energysystem"}) for scenario in scenarios
        ]
        return DataResponse(
...
async def delete_scenario_endpoint(
    ...
    if not user.check_scenario_rights(scenario_id):
        ...
        delete_scenario(scenario_id=scenario_id, user=user)
...
async def duplicate_scenario_endpoint(
    ...
    scenario = duplicate_scenario(scenario_id=scenario_id, user=user)
